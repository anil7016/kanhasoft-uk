...
wp_enqueue_style( 'custome-style', get_template_directory_uri() . '/css/custom-style.css',false,'1.1','all');
// Custom javascript
...
var navbar = document.getElementById("mega-menu-header-section");
...
function myFunction() {
  if (window.pageYOffset >= sticky) {
    ...
    navbar.classList.remove("sticky");
		 ...
		 function reveal() {
    const reveals = document.querySelectorAll(".reveal-animation-rollin");
  ...
  window.addEventListener("scroll", reveal);
	 ...
	 <script>	
		 function reveal() {
    const reveals = document.querySelectorAll(".reveal-animation-zoomin");
    for (let i = 0; i < reveals.length; i++) {
      const windowHeight = window.innerHeight;
      const elementTop = reveals[i].getBoundingClientRect().top;
      const elementVisible = 150;
